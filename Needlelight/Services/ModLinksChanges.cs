...
public class ModLinksChanges : IModLinksChanges
{
    ...
    private readonly NeedlelightMode NeedlelightMode;
    ...
    public ModLinksChanges(IEnumerable<ModItem> _items, ISettings _settings, NeedlelightMode _NeedlelightMode)
    {
        currentItems = _items;
        settings = _settings;
        NeedlelightMode = _NeedlelightMode;
    ...
    {
        // only do it if its not offline and Needlelight is using hk-modding/modlinks as its modlinks provider
        if (NeedlelightMode == NeedlelightMode.Offline ||
        ...
        try
        {
            JsonDocument linkJson = JsonDocument.Parse(
                await hc.GetStringAsync2(
                    settings,
                    new Uri("https://raw.githubusercontent.com/TheMulhima/Needlelight/static-resources/ModlinksChanges.json"),
