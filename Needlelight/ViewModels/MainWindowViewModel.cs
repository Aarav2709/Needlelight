...
namespace Needlelight.ViewModels
{
  ...
  public string AppName => "Needlelight";
    ...
    public string ActiveProfileName
    {
      get
      {
        try
        {
          ...
          return Needlelight.Models.GameProfiles.GetByKey(key).Name;
        }
        catch
        {
          return Needlelight.Models.GameProfiles.HollowKnight.Name;
        }
      }
    }
  ...
  Trace.WriteLine($"Opening Needlelight Version: {Assembly.GetExecutingAssembly().GetName().Version}");
  ...
  hc.DefaultRequestHeaders.Add("User-Agent", "Needlelight");
      ...
      await MessageBoxUtil.GetMessageBoxStandardWindow(
        ...
        $"Needlelight was unable to load modlinks from {settings.CustomModlinksUri}. Needlelight will load official modlinks instead.",
                icon: Icon.Error
        ...
        hc = res.Client;
        NeedlelightMode = NeedlelightMode.Online;
      ...
      catch (Exception e) when (e is TaskCanceledException { CancellationToken.IsCancellationRequested: true } or HttpRequestException)
      {
        ...
        NeedlelightMode = NeedlelightMode.Offline;
      }
    ...
    private async Task HandleURLSchemeCommand(IUrlSchemeHandler urlSchemeHandler)
    {
      ...
      if (!urlSchemeHandler.Handled && urlSchemeHandler.UrlSchemeCommand != UrlSchemeCommands.none)
      {
        var prompt = urlSchemeHandler.UrlSchemeCommand switch
        {
          ...
          UrlSchemeCommands.reset => $"Reset Needlelight's persistent settings",
      ...
      if (!urlSchemeHandler.Handled)
      {
        ...
        if (urlSchemeHandler.UrlSchemeCommand == UrlSchemeCommands.useOfficialModLinks)
        {
          ...
          Dispatcher.UIThread.InvokeAsync(async () => await urlSchemeHandler.ShowConfirmation(
              ...
              message: "Needlelight will now use official modlinks"));
        }
        ...
        while (!configAccessSuccess)
        {
          ...
          catch (IOException)
          {
            ...
            (
                ...
                text: $"Needlelight cannot run without being able to access {cfgPath}.\n" +
  ...
  // Previously `profile` was declared after being used which caused CS0103/CS0841 errors.
    ...
    public async Task SwitchGameAsync(string key)
    {
      try
      {
        ...
        var selectedProfile = Needlelight.Models.GameProfiles.GetByKey(key);
