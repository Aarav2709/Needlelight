...
namespace Needlelight.ViewModels
{
  public partial class ModListViewModel : ViewModelBase
  {
    ...
    private readonly NeedlelightMode _NeedlelightMode;
    ...
    public ModListViewModel(
        ...
        NeedlelightMode NeedlelightMode)
    {
      ...
      _NeedlelightMode = NeedlelightMode;
      ...
      _modlinksChanges = new ModLinksChanges(_items, _settings, _NeedlelightMode);
      ...
      if (_NeedlelightMode == NeedlelightMode.Offline) SelectModsWithFilter(ModFilterState.Installed);
  ...
  // banner removed per user request
    ...
    private async Task HandleLocationURLScheme()
    {
      if (_urlSchemeHandler is { Handled: false, UrlSchemeCommand: UrlSchemeCommands.location })
      {
        ...
        await _urlSchemeHandler.ShowConfirmation(
            ...
            message: sucess ? $"Needlelight has opened {_urlSchemeHandler.Data}" : $"Needlelight was unable to open {_urlSchemeHandler.Data}. Please check if the command is correct",
    ...
    private async Task HandleDownloadUrlScheme()
    {
      if (_urlSchemeHandler is { Handled: false, UrlSchemeCommand: UrlSchemeCommands.download })
      {
        ...
        if (successfulDownloads.Count > 0)
          message +=
              $"Needlelight has successfully downloaded {string.Join(Resources.Array_Sep, successfulDownloads)} from command";

        if (failedDownloads.Count > 0)
        {
          message +=
              $"However, Needlelight was unable to download {string.Join(Resources.Array_Sep, failedDownloads)} from command. Please check if the command is correct";
        }
    ...
    private async Task HandleForceUpdateAllScheme()
    {
      if (_urlSchemeHandler is { Handled: false, UrlSchemeCommand: UrlSchemeCommands.forceUpdateAll })
      {
        ...
        await _urlSchemeHandler.ShowConfirmation(
            ...
            message: "Needlelight has successfully run force updated all command");
      }
    }
    private async Task HandleRemoveGlobalSettingScheme()
    {
      if (_urlSchemeHandler is { Handled: false, UrlSchemeCommand: UrlSchemeCommands.removeGlobalSettings })
      {
        ...
        if (successfulDownloads.Count > 0)
          message +=
              $"Needlelight has removed global settings for {string.Join(Resources.Array_Sep, successfulDownloads)} from command";

        if (failedDownloads.Count > 0)
        {
          message +=
              $"However, Needlelight was unable to find the global settings for {string.Join(Resources.Array_Sep, failedDownloads)}. Please check if the mod name is correct and the mod is installed";
        }
    ...
    public bool IsInOnlineMode => _NeedlelightMode == NeedlelightMode.Online;
  ...
  public bool CanUpdateAll => _items.Any(x => x.State is InstalledState { Updated: false }) && !_updating;
    ...
    public static async Task ToggleApiCommand(IModSource _mods, IInstaller _installer)
    {
      async Task<bool> DoActionWithWithErrorHandling(Func<Task> Action)
      {
        ...
        {
          await DisplayErrors.AskForAdminReload($"Needlelight failed to {Resources.MVVM_Install} API");
      ...
      if (_mods.ApiInstall is InstalledState installedState)
      {
        ...
        /* this accounts for edge case when Needlelight says api is installed (installedState.Enabled) when its not (!isApiActuallyInstalled)
        so we first install to ensure Needlelight's state matches with reality and then we toggle to do what user wants
        */
    ...
    public async Task OnEnable(object itemObj, bool checkForDependencies)
    {
      ...
      {
        await DisplayErrors.AskForAdminReload($"Needlelight failed to toggle {item}");
    ...
    public async Task InternalModDownload(ModItem item, Func<IInstaller, Action<ModProgressArgs>, Task> downloader)
    {
      ...
      {
        await DisplayErrors.AskForAdminReload($"Needlelight failed to download {item.Name}");
